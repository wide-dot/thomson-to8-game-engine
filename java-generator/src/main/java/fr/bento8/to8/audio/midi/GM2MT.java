...
public class GM2MT {
	...
	private static final int PERC_CHANNEL = 9;
	
	public static void convert_to_mt(MidiMessage message) throws InvalidMidiDataException
	{	
		switch(((ShortMessage)message).getCommand())
		{
		    case ShortMessage.PROGRAM_CHANGE:
			/* remap ins */
			((ShortMessage)message).setMessage(((ShortMessage)message).getCommand(), ((ShortMessage)message).getChannel(), midi_gm2mt[((ShortMessage)message).getData1() + 1] - 1, 0);
			break;			
			
		    case ShortMessage.NOTE_ON:
			if (((ShortMessage)message).getChannel() == PERC_CHANNEL)
			{
				/* remap perc */
				((ShortMessage)message).setMessage(((ShortMessage)message).getCommand(), ((ShortMessage)message).getChannel(), perc_gm2mt[((ShortMessage)message).getData1() + 1] - 1, ((ShortMessage)message).getData2());
				break;
			}
			break;
		}
	...
	public static final int midi_gm2mt[] = {
		-1,
	...
	public static final int perc_gm2mt[] = {
	    0,
	    1,
		2,
		3,
