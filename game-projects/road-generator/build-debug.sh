#!/usr/bin/env bash
# ============================================================================
# build-debug.sh — Build le game-mode DEBUG "road-debug" (validation placement)
# ----------------------------------------------------------------------------
# Identique à build.sh mais pointe sur config-macos-debug.properties
# (gameModeBoot=roaddebug). Produit dist/road-generator-debug.fd.
#
# Usage :
#   ./build-debug.sh           → génère le .fd debug
#   ./build-debug.sh --clean   → force le rebuild du jar Java
#   ./build-debug.sh --run     → après build, lance dans dcmoto si dispo
#
...
CONFIG="./config-macos-debug.properties"
...
cd "$SCRIPT_DIR"
echo "==> Génération road-generator-debug depuis $CONFIG ..."
...
if [ $RUN -eq 1 ]; then
    ...
    if [ -L "$APP_LINK" ] || [ -d "$APP_LINK" ]; then
        ...
        echo "==> Aucun dcmoto.app trouvé. dist/road-generator-debug.fd prêt."
